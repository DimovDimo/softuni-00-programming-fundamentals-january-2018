    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (leftMoreBestDNAs.Count == 1)
            {
                ...
                var splitedFirst = leftMoreBestDNAs[0]
                    .Split(new char[] { '!' }, StringSplitOptions.RemoveEmptyEntries)
                    .ToList();
                var DNARex = string.Join("", splitedFirst);
                MatchCollection matchesDNA = Regex.Matches(DNARex, @"(1+)");
                var bestLenghtCurrnet = 0;
                foreach (Match item in matchesDNA)
                {
                    if (item.Groups[1].Value.Length > bestLenghtCurrnet)
                    {
                        bestSumNow = item.Groups[1].Value.Length;
                    }
                }
